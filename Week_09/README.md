...
### Пълен пример — `ls -l` поведение за един файл
...
int main(int argc, char *argv[]) {
    ...
    dprintf(STDOUT_FILENO, "%c%s %2lu %-8s %-8s %8ld %s",
        type, perms,
        (unsigned long)st.st_nlink,
        pw ? pw->pw_name : "?",
        gr ? gr->gr_name : "?",
    ...
    // Ако е symlink — покажи накъде сочи
    if (S_ISLNK(st.st_mode)) {
        char target[512];
        ssize_t len = readlink(argv[1], target, sizeof(target) - 1);
        if (len != -1) {
            target[len] = '\0';
            dprintf(STDOUT_FILENO, " -> %s", target);
        }
    }
