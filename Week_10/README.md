...
pid_t fork(void);
// Извиква се веднъж, но се ВРЪЩА ДВАПЪТИ:
//   В родителя → връща PID на детето (положително число)
//   В детето   → връща 0
//   При грешка → връща -1 (само в родителя, дете не се създава)
```
...
Преди fork():
  Родител (PID 100)
  [код] [данни] [стек] [FD таблица]

fork() се изпълнява:
  ядрото копира адресното пространство (COW)
  ядрото копира FD таблицата
  ядрото задава ppid на детето = 100

След fork():
  Родител (PID 100)          Дете (PID 101)
  fork() връща 101           fork() връща 0
  [код] [данни] [стек]      [код] [данни] [стек]  ← идентични (COW)
  [FD: 0,1,2,...]           [FD: 0,1,2,...]        ← копирани
```

### Основна употреба — разграничаване с PID
...
int main(void) {
    ...
    if (pid == 0) {
        ...
        // ... детето прави своята работа ...
        _exit(0);  // _exit() вместо exit() в дете след fork
    }

    // ── Тук сме в РОДИТЕЛЯ ──
    dprintf(STDOUT_FILENO, "Родител: PID=%d, детето е %d\n", getpid(), pid);

    // Изчакай детето да завърши
    int status;
...
`pid_t` е знаков целочислен тип (обикновено `int32_t`), дефиниран в `<unistd.h>`. Използва се за всички PID стойности:

```c
pid_t pid    = fork();    // резултат от fork
pid_t my     = getpid();  // PID на текущия процес
pid_t parent = getppid(); // PID на родителя
```

Знаковият тип позволява `-1` за грешка и `0` за детето.
...
```c
// exit() — извиква atexit() handlers, flush-ва stdio буфери
//           → може да причини двоен flush ако родителят и детето споделят буфери

// _exit() — директен syscall, без cleanup
...
int main(void) {
    for (int i = 0; i < NUM_WORKERS; i++) {
        ...
        if (pid == 0) {
            // Всяко дете обработва своята задача
...
`exec()` **заменя текущия процес с нова програма**. За разлика от `fork()`, `exec()` не създава нов процес — PID остава същият, но кодът, данните и стекът се презаписват с новата програма.

Ако `exec()` успее, **не се връща никога** — текущата програма е вече заменена. Връща `-1` само при грешка.
...
`fork()` и `exec()` рядко се използват поотделно — почти винаги се комбинират. Това е **основният pattern за стартиране на нова програма** в Unix/Linux:

```
1. fork()  → създай копие на текущия процес
2. exec()  → в детето замени програмата с желаната
3. wait()  → родителят чака детето да завърши
```

```
Родител (bash, PID 100)
    │
    ├── fork() ──────────────────────► Дете (PID 101)
    │                                       │
    │   (чака с wait())              execvp("ls", args)
    │                                       │
    │                               ls (PID 101) се изпълнява
    │                                       │
    │◄──────────────────────────────── exit(0)
    │
    │   wait() се връща
...
Именно така работи всеки shell: когато напишеш команда, bash прави `fork()`, детето прави `exec()` на командата, а bash чака с `wait()`.

### Пренасочване на FD между fork и exec

Тъй като детето наследява FD таблицата от родителя, можеш да **пренасочиш stdin/stdout/stderr преди exec()** — без програмата, която ще изпълниш, да знае нищо за това. Точно така shell-ът имплементира `>`, `<` и `|`:

```c
pid_t pid = fork();
if (pid == 0) {
    // Пренасочи stdout към файл
    int fd = open("output.txt", O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (fd == -1)
        err(1, "open");

    // Дублирай fd върху STDOUT_FILENO (FD 1)
    if (dup2(fd, STDOUT_FILENO) == -1)
        err(1, "dup2");

    close(fd);  // оригиналният fd вече не е нужен

    // ls ще пише в output.txt без да знае
    execlp("ls", "ls", "-l", NULL);
    err(1, "execlp");
}
```
...
if (WIFEXITED(status)) {
    // Детето е завършило нормално с exit()
    int code = WEXITSTATUS(status);  // exit кодът (0–255)
